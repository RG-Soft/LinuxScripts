...
for dbname in $dblist ; do
    ...
    table_list=`/opt/pgpro/std-15/bin/psql --dbname=$dbname --host=$hostname --port=$port --username=$username --command="copy ($select_tables) to stdout"`

    for table_name in $table_list ; do
	echo "	Таблица в обработке \"$table_name\""
	/opt/pgpro/std-15/bin/pg_repack --host=$hostname --port=$port --username=$username --no-password --dbname=$dbname --table=$table_name
    done

    # Получаем список объектов к обработке индексов
    echo "	Подсчитываем статистику по индексам..."

    index_list=`/opt/pgpro/std-15/bin/psql --dbname=$dbname --host=$hostname --port=$port --username=$username --command="copy ($select_indexes) to stdout"`

    for index_name in $index_list ; do
	echo "	Индекс в обработке \"$index_name\""
	/opt/pgpro/std-15/bin/pg_repack --host=$hostname --port=$port --username=$username --no-password --dbname=$dbname --index=$index_name
    done
done
