...
clustername=slbSVS_acc # можно использовать номер порта, если кластен не именной, по имени базы.
srvname=localhost
port=5434
username=postgres
backupdir_root=/pgbackup

/var/lib/postgresql/basebackup_cluster.sh $srvname $port $username $clustername $backupdir_root
